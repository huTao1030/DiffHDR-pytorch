...
def generalized_steps(x, x_cond, seq, model, attenfeature, b, eta=0.):
    with torch.no_grad():
        n = x.size(0)
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            et = model(torch.cat([x_cond, xt], dim=1), t, attenfeature)
...
def generalized_steps_overlapping(x, x_cond, seq, model, ahdr, b, eta=0., corners=None, p_size=None, manual_batching=True):
    with torch.no_grad():
        n = x.size(0)
        seq_next = [-1] + list(seq[:-1])
        x0_preds = []
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            if manual_batching:
                ...
                for i in range(0, len(corners), manual_batching_size):
                    attenfeature =ahdr(x_cond_patch[i:i+manual_batching_size][:, :6, :, :],x_cond_patch[i:i+manual_batching_size][:, 6:12, :, :],x_cond_patch[i:i+manual_batching_size][:, 12:18, :, :])
                    outputs = model(torch.cat([x_cond_patch[i:i+manual_batching_size],
                                               xt_patch[i:i+manual_batching_size]], dim=1), t, attenfeature)
                    for idx, (hi, wi) in enumerate(corners[i:i+manual_batching_size]):
                        et_output[0, :, hi:hi + p_size, wi:wi + p_size] += outputs[idx]
            else:
                for (hi, wi) in tqdm(corners,total=len(corners)):
